	...
	private T _deserializeInt(int id, DeserializationContext ctxt) throws JsonProcessingException {
		...
		if (this.findByIdMethod != null) {
			try {
				...
				throw JsonMappingException.from(ctxt.getParser(), "invoke findById method error, problem: " + e.getMessage(), e);
			}
		} else {
		...
		if (rst == null && !ctxt.isEnabled(DeserializationFeature.READ_UNKNOWN_ENUM_VALUES_AS_NULL)) {
			throw ctxt.weirdNumberException(id, handledType(), "value not one of declared IntEnum instance numberss: " + id);
		}
		return rst;
	}
		...
		if (curr == JsonToken.START_ARRAY && ctxt.isEnabled(DeserializationFeature.UNWRAP_SINGLE_VALUE_ARRAYS)) {
			...
			if (curr != JsonToken.END_ARRAY) {
				throw ctxt.wrongTokenException(jp, JsonToken.END_ARRAY,
						"Attempted to unwrap single value array for single '" + handledType().getName() + "' value but there was more than a single value in the array");
			}
			return parsed;
		}
